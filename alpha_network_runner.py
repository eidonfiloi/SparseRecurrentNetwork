...
if __name__ == "__main__":
    ...
    text_data_path = "data_prepared/text_test.txt" #"data_prepared/wiki_100k.txt"
                ...
                if i < 50:
                    ...
                    for el in arr:
                        ...
                        if prev_pred is not None:
                            ...
                            plt.ion()
                            plt.axis([-1, 16, -1, 16])
                            x_r, y_r = np.argwhere(inp.reshape(16, 16) == 1).T
                            x_t, y_t = np.argwhere(prev_pred_vector.reshape(16, 16) == 1).T
                            plt.scatter(x_r, y_r, alpha=0.5, c='r', marker='s', s=155)
                            plt.scatter(x_t, y_t, alpha=0.5, c='b', marker='o', s=145)
                            plt.draw()
                            time.sleep(0.1)
                            plt.clf()
                        prev_pred = copy(pred)
